...
def sample_drellyan_maf(model, n_samples=1000, batch_size=500, max_iter=50):
    model.eval()
    collected = []

    for it in range(max_iter):
        # Propose batch of latent Gaussian samples
        u = torch.randn(batch_size, 6)
        samples, log_det = model.backward(u)

        # Filter invalid values
        samples = samples.detach().cpu()
        mask = torch.isfinite(samples).all(dim=1)  # keep only finite rows
        valid_samples = samples[mask]

        collected.append(valid_samples)

        total = sum(len(x) for x in collected)
        print(f"[Iter {it+1}] Collected {total}/{n_samples} samples")
    ...
    if len(collected) < n_samples:
        print(f"⚠️ Warning: only collected {len(collected)} valid samples "
...
#    samples, log_det = model.backward(u)
#
#    # Remove NaNs across the whole tensor
#    mask = ~torch.isnan(samples)
#    samples = samples[mask.all(dim=1)]  # keep only rows where all features are valid
#
#    samples = samples.detach().cpu()
#    return samples
